...
<div class="wrapper">
    <div class="header">
        ...
        <p>Formulaire de téléchargement du guide, reçu le {{ $guideDownload->created_at->format('d/m/Y à H:i') }}</p>
        <div class="badge">Prospect #{{ $guideDownload->id }}</div>
    </div>
    <div class="body">
        <div class="grid">
            <div class="field">
                <div class="field-label">Organisation</div>
                <div class="field-value">{{ $guideDownload->organisation }}</div>
            </div>
            <div class="field">
                <div class="field-label">Responsabilité</div>
                <div class="field-value">{{ $guideDownload->fonctionLabel() }}</div>
            </div>
            <div class="field">
                <div class="field-label">Pays</div>
                <div class="field-value">{{ $guideDownload->pays }}</div>
            </div>
            @if($guideDownload->taille_organisation)
            <div class="field">
                <div class="field-label">Taille de l'organisation</div>
                <div class="field-value">{{ \App\Models\GuideDownload::TAILLES[$guideDownload->taille_organisation] ?? $guideDownload->taille_organisation }}</div>
            </div>
            @endif
            @if($guideDownload->nombre_projets)
            <div class="field">
                <div class="field-label">Projets gérés actuellement</div>
                <div class="field-value">{{ $guideDownload->nombre_projets }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="field">
            ...
            <div class="field-value">{{ $guideDownload->nom }}</div>
            <div class="field-value"><a href="mailto:{{ $guideDownload->email }}" style="color:#1e40af;">{{ $guideDownload->email }}</a></div>
            @if($guideDownload->telephone)
            <div class="field-value"><a href="tel:{{ $guideDownload->telephone }}" style="color:#1e40af;">{{ $guideDownload->telephone }}</a></div>
            @endif
        </div>

        <div class="consent {{ $guideDownload->consentement_marketing ? 'consent-yes' : 'consent-no' }}">
            {{ $guideDownload->consentement_marketing ? '✓ A accepté d\'être contacté(e) au sujet de Finance Pro.' : '✗ N\'a PAS souhaité être contacté(e) — respecter ce choix pour toute relance commerciale.' }}
        </div>

        <div class="actions">
            <a href="mailto:{{ $guideDownload->email }}?subject=Finance Pro — suite à votre téléchargement du guide" class="btn">✉️ Relancer par email</a>
        </div>
    </div>
    <div class="footer">
