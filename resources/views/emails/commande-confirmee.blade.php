...
<div class="wrapper">
    ...
    <div class="header">
        ...
        <div class="badge">✓ Commande {{ $commande->numero_commande }}</div>
    </div>

    <!-- Body -->
    <div class="body">
        <p class="greeting">Bonjour {{ $commande->client_nom }},</p>
        <p class="text">
        ...
        <div class="card">
            ...
            <div class="row">
                ...
                <span class="value">{{ $commande->service?->nom }}</span>
            </div>
            @if($commande->service?->duree)
            <div class="row">
                <span class="label">Durée estimée</span>
                <span class="value">{{ $commande->service->duree }}</span>
            </div>
            @endif
            <div class="row">
                <span class="label">Date de paiement</span>
                <span class="value">{{ $commande->paiement_at?->format('d/m/Y à H:i') }}</span>
            </div>
            <div class="row">
                <span class="label">Méthode de paiement</span>
                <span class="value">{{ ucfirst(str_replace('_', ' ', $commande->methode_paiement)) }}</span>
            </div>
            <hr class="divider">
            <div class="row">
                <span class="label">Montant HT</span>
                <span class="value">{{ number_format($commande->montant_fcfa, 0, ',', ' ') }} FCFA</span>
            </div>
            <div class="row">
                <span class="label">AIB (5%)</span>
                <span class="value">{{ number_format($commande->tva_fcfa, 0, ',', ' ') }} FCFA</span>
            </div>
            <div class="total-row">
                <span class="label">Total payé</span>
                <span class="value">{{ number_format($commande->total_ttc_fcfa, 0, ',', ' ') }} FCFA</span>
            </div>
        </div>
        ...
        <div class="btn-center">
            <a href="{{ config('app.url') }}/paiement/succes/{{ $commande->numero_commande }}" class="btn">
