...
<head>
    <meta charset="UTF-8">
    <title>Nouveau message – {{ $contact->sujet }}</title>
    <style>
...
<div class="wrapper">
    <div class="header">
        ...
        <div class="badge">Réf. {{ $contact->reference }}</div>
    </div>
    <div class="body">
        <div class="field">
            <div class="field-label">Expéditeur</div>
            <div class="field-value">{{ $contact->nom }}</div>
        </div>
        <div class="field">
            <div class="field-label">Email</div>
            <div class="field-value"><a href="mailto:{{ $contact->email }}" style="color:#1e40af;">{{ $contact->email }}</a></div>
        </div>
        @if($contact->telephone)
        <div class="field">
            <div class="field-label">Téléphone</div>
            <div class="field-value"><a href="tel:{{ $contact->telephone }}" style="color:#1e40af;">{{ $contact->telephone }}</a></div>
        </div>
        @endif
        @if($contact->entreprise)
        <div class="field">
            <div class="field-label">Organisation</div>
            <div class="field-value">{{ $contact->entreprise }}</div>
        </div>
        @endif
        <div class="field">
            <div class="field-label">Sujet</div>
            <div class="field-value">{{ $contact->sujet }}</div>
        </div>
        <div class="field">
        ...
        <div class="actions">
            <a href="mailto:{{ $contact->email }}?subject=Re: {{ $contact->sujet }}&body=Bonjour {{ $contact->nom }},%0D%0A%0D%0A" class="btn">
                ✉️ Répondre
            </a>
            <a href="{{ config('app.url') }}/admin/contacts" class="btn" style="background:#475569;">
                Voir dans l'admin
            </a>
        </div>
    </div>
