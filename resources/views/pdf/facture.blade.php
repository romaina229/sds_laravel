...
<div class="container">
    ...
    <div class="header">
        ...
        <div class="invoice-info">
            <h2>Facture</h2>
            <p class="invoice-number">{{ $facture->numero_facture }}</p>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>
                    <strong>{{ $commande->service_nom }}</strong><br>
                    <small style="color:#666">Commande N° {{ $commande->numero_commande }}</small>
                </td>
                <td>{{ $commande->duree_estimee ?? '-' }}</td>
                <td style="text-align:right">{{ number_format($commande->montant_fcfa, 0, ',', ' ') }} FCFA</td>
                <td style="text-align:right">{{ number_format($commande->montant_euro, 2, ',', ' ') }} €</td>
            </tr>
        </tbody>
        <tfoot>
            <tr style="background:#f8f9fa">
                <td colspan="2" style="text-align:right">Montant HT :</td>
                <td colspan="2" style="text-align:right">{{ number_format($commande->montant_fcfa, 0, ',', ' ') }} FCFA</td>
            </tr>
            <tr style="background:#f8f9fa">
                <td colspan="2" style="text-align:right">AIB ({{ $taux_aib * 100 }}%) :</td>
                <td colspan="2" style="text-align:right">{{ number_format($commande->tva_fcfa, 0, ',', ' ') }} FCFA</td>
            </tr>
            <tr class="total-row">
                <td colspan="2" style="text-align:right;font-size:14px;">TOTAL TTC :</td>
                <td colspan="2" style="text-align:right;font-size:16px;font-weight:900;">{{ number_format($commande->total_ttc_fcfa, 0, ',', ' ') }} FCFA</td>
            </tr>
        </tfoot>
    </table>

    <div class="aib-note">
        <strong>Note fiscale :</strong> AIB (Acompte sur Impôt sur les Bénéfices) de {{ $taux_aib * 100 }}% appliquée conformément à la législation fiscale du Bénin.
    </div>

    <!-- Paiement info -->
    <p style="margin-bottom:5px;font-size:11px;color:#555;">
        <strong>Méthode de paiement :</strong> {{ $commande->methode_paiement_label }}
        @if($commande->paiement_at)
        | <strong>Date de paiement :</strong> {{ $commande->paiement_at->format('d/m/Y à H:i') }}
        @endif
    </p>

    <!-- Footer -->
    <div class="footer">
        <p>Merci pour votre confiance ! | Shalom Digital Solutions - {{ date('Y') }}</p>
